...
mod off_inheritance_tests {
    ...
    fn test_inline_off_removes_inherited_middleware() {
        ...
        let middleware_names: Vec<String> = inline_strategy.middleware.iter()
    ...
    fn test_inline_partial_off() {
        ...
        site.routes[0] = Route::Static {
            ...
            strategy: Some(StrategyRef::InlineMiddleware(vec![
                ...
                MiddlewareConfig::new_named_json(                     // Enabled
                    "cors".to_string(),
                    json!({"origins": ["*"]})
                )
            ])),
        ...
        assert!(middleware_names.contains(&"cors".to_string()), "cors should be present");
        assert!(middleware_names.contains(&"logging".to_string()), "logging should be present");
        
        // Should NOT contain rate_limit (disabled in inline)
        assert!(!middleware_names.contains(&"rate_limit".to_string()), "rate_limit should be removed");
