...
mod user_scenario_test {
    ...
    fn test_user_exact_scenario() {
        ...
        assert!(
            logging_middleware.is_some(),
            "logging should be present from inheritance"
        );

        println!("✅ User exact scenario works correctly - rate_limit is completely removed");
        println!(
            "   This matches the expected behavior: inline 'rate_limit: off' removes it completely"
        );
    }
    ...
    fn test_strategy_resolver_keeps_on_middleware_with_empty_config() {
        ...
        assert_eq!(
            site_strategy.middleware[0].config_as_json().unwrap(),
            json!({})
        );
        ...
        assert_eq!(
            route_strategy.middleware[0].config_as_json().unwrap(),
            json!({})
        );
