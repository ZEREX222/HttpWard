...
mod comprehensive_inheritance_tests {
    ...
    fn test_complex_inheritance_priority() {
        ...
        let rate_limit = complex_strategy
            .middleware
            .iter()
            .find(|m: &&MiddlewareConfig| m.name() == "rate_limit")
            .unwrap();
    ...
    fn test_no_inheritance_when_no_strategies() {
        ...
        println!(
            "Debug - standalone middleware count: {}",
            standalone_strategy.middleware.len()
        );
        ...
        let config = standalone_strategy
            .middleware
            .iter()
