...
mod hierarchical_inheritance_tests {
    ...
    fn create_global_with_all_middleware() -> GlobalConfig {
        GlobalConfig {
            ...
            strategies: {
                ...
                strategies.insert("global_base".to_string(), vec![
    ...
    fn create_site_with_partial_override() -> SiteConfig {
        SiteConfig {
            ...
            strategies: {
                let mut strategies = std::collections::HashMap::new();
                strategies.insert("site_override".to_string(), vec![
    ...
    fn test_hierarchical_global_to_site_merging() {
        ...
        println!("Debug - site strategy middleware count: {}", site_strategy.middleware.len());
        ...
        let cache = site_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "cache").unwrap();
        ...
        let security = site_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "security").unwrap();
    ...
    fn test_hierarchical_inline_with_full_inheritance() {
        ...
        assert!(inline_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "logging").is_none());
        
        // Check cors: inherited from global
        let cors = inline_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "cors").unwrap();
        assert!(cors.config_as_json().is_ok());
        
        // Check security: inherited from site
        let security = inline_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "security").unwrap();
        let sec_config = security.config_as_json().unwrap();
        assert_eq!(sec_config["site_security"], "site_security");
        
        // Check monitoring: only from inline
        let monitoring = inline_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "monitoring").unwrap();
    ...
    fn test_hierarchical_route_local_strategy_isolation() {
        ...
        site.routes.push(Route::Proxy {
            ...
            strategies: Some({
                ...
                local_strats.insert("local_isolated".to_string(), vec![
        ...
        let cors = local_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "cors").unwrap();
        assert!(cors.is_off());
        
        // Check local_only: only from local
        let local_only = local_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "local_only").unwrap();
        assert!(local_only.config_as_json().is_ok());
        
        // Should NOT have site or global middleware
        assert!(local_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "logging").is_none());
        assert!(local_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "security").is_none());
    ...
    fn test_hierarchical_deep_inheritance_chain() {
        ...
        site.routes.push(Route::Proxy {
            ...
            strategies: Some({
                ...
                deep_strats.insert("deep_route".to_string(), vec![
        ...
        println!("Debug - deep strategy middleware count: {}", deep_strategy.middleware.len());
        ...
        let cache = deep_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "cache").unwrap();
        ...
        let compression = deep_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "compression").unwrap();
        ...
        assert!(deep_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "rate_limit").is_none());
        assert!(deep_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "logging").is_none());
    ...
    fn test_hierarchical_with_off_at_different_levels() {
        ...
        site.strategies.get_mut("site_override").unwrap()[1] = MiddlewareConfig::new_off("logging".to_string());
        ...
        let logging = off_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "logging").unwrap();
        let log_config = logging.config_as_json().unwrap();
        ...
        assert!(off_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "auth").is_none());
        
        // Check rate_limit: from site (which overrode global)
        let rate_limit = off_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "rate_limit").unwrap();
    ...
    fn test_hierarchical_empty_inheritance_levels() {
        ...
        let rate_limit = empty_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "rate_limit").unwrap();
        let config = rate_limit.config_as_json().unwrap();
        ...
        let cors = empty_strategy.middleware.iter().find(|m: &&MiddlewareConfig| m.name() == "cors").unwrap();
        let cors_config = cors.config_as_json().unwrap();
        assert_eq!(cors_config["global_cors"], "global_cors");
    ...
    fn test_hierarchical_strategy_resolution_priority_matrix() {
        ...
        let test_cases = vec![
            ...
            (Some(StrategyRef::Named("global_base".to_string())), 5, "Route uses global directly"),
            (Some(StrategyRef::Named("site_override".to_string())), 6, "Route uses site (merged with global)"),
            (Some(StrategyRef::InlineMiddleware(vec![
                MiddlewareConfig::new_named_json("test".to_string(), json!({"value": 1}))
            ])), 7, "Route uses inline (inherits from site+global)"),
        ...
        for (i, (route_strategy, expected_count, description)) in test_cases.into_iter().enumerate() {
            ...
            assert_eq!(strategy.middleware.len(), expected_count, 
                "Failed for {}: expected {}, got {}", description, expected_count, strategy.middleware.len());
        }
