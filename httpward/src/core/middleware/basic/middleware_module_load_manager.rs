use super::middleware_global_module_storage::{
    ModuleRecord, initialize_global_storage, with_global_storage,
};
...
impl MiddlewareModuleLoadManager {
    ...
    pub fn from_server_instances_statically(
        ...
        Self::from_server_instances_with_dir_statically(
            server_instances,
            Path::new("modules"),
            static_modules,
        )
    ...
    pub fn from_server_instances_with_dir<P: AsRef<Path>>(
        ...
        let mut manager = Self { modules_dir };
    ...
    fn load_dynamic_record(
        &self,
        name: &str,
    ) -> Result<ModuleRecord, Box<dyn std::error::Error + Send + Sync>> {
    ...
    fn load_module_by_name_and_add_to_global(
        &mut self,
        name: &str,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        Self::ensure_storage()?;
